...
- ⭐**매물 목록은 POST 가 주 경로**(2026-08-08 전환): `POST fin.land
  /front-api/v1/complex/article/list`, body `{complexNumber:int, tradeTypes:['A1'|'B1'|'B2'],
  page, size}`. 예전에 "스키마가 까다로워 400"으로 포기했던 그 엔드포인트인데,
  **tradeTypes 를 반드시 넣어야** 200 이 온다(빠지면 400). 응답 항목 구조는 GET 과
  같아 `_parse_naver_article` 을 그대로 쓴다. 효과 2가지:
  ① 동×평형 GET 순회(최대 15동×5평형=**75요청**)가 거래유형당 1~2요청(**3요청**)으로 —
     차단 위험이 급감(예전엔 70개 단지에서 429 중단).
  ② **'집계는 42건인데 상세 0건'이던 단지가 정상 수집**된다(현대프라임 complexNo 84 —
     코드에 "네이버 노출제한 추정"으로 남아 있던 미해결 건. POST 로는 매매 23·전세 17·
     월세 2 전량 수집 확인, 네이버 웹 화면과 일치). GET 순회는 실패 시 폴백으로만 남김.
...
- **상세 패널 이름 옆 종합점수 배지(2026-08-08)**: `SeoulDash.scoreBadge(id)` —
  현재 프리셋 × **전체(대표평형) 기준** 전 단지 순위를 1회 계산·캐시(프리셋 바뀌면
  재계산). 점수 패널의 평형·구 필터와 무관하게 항상 전체 서울 순위(필터 따라
  출렁이면 배지로 부적합). 순위 제외 단지(2축 이하)는 **'순위 밖'** 으로 명시.
  렌더는 app.js renderPanel 이 비동기 주입(`#p-score`, 다른 단지로 넘어가면 폐기).
  같은 날 점수 패널 표시도 100위 컷 → **더보기 +200씩**으로 전환(scLimit).
...
## 주차(지하주차장 필터, 2026-08-08 신설)

- 건축물대장에 **'지하주차장' 항목은 없다** → **옥내 주차대수>0** 을 프록시로 쓴다
  (아파트에서 옥내 주차 = 사실상 지하주차장). 컬럼 `park_total/park_indr/park_oudr`
  (+ `park_fetched_at` = 백필 대상 선별 마커), 마커 export 는 `pki`(옥내)·`pkt`(총).
  세대당 대수 = `pkt/hh`. UI: 상세 패널 헤더 아래 한 줄 + 필터 "지하주차장 있는 단지만".
- **3분류를 뭉개지 말 것**: 총 대수만 있고 옥내/옥외 내역이 비어 오는 대장이 있다
  (실측: 극동 총 18·내역 0). 이걸 `indr=0` 으로 저장하면 '지상주차만'으로 **단정**되어
  필터에서 잘못 걸러진다 → 내역은 `None`(모름)으로 둔다. 지하O 7,700 / 지상만 550 /
  모름 58 / 미기재 1,235(구축 다수) — 커버리지 87%.
- 백필: `building --parking`. **이미 수집된 단지를 재조회**하므로 `db.set_parking` 이
  주차 컬럼만 쓴다 — `set_building` 을 쓰면 조회가 빈손일 때 기존 세대수·용적률이
  통째로 NULL 이 된다(수집 6주치 소실 위험).
- ⚠**건축물대장 API 는 키 단위 초당 제한**이 있다. 워커를 늘리면 3건 중 1건이 429 로
  실패하고 재시도로 되돌아와 **오히려 느려진다**(실측: 워커 6→13건/분, 10→20, 28→38).
  스레드 수와 무관한 **전역 호출 간격 0.2s**(`_RateLimiter`, ≈5 req/s)를 넣자
  429 0건 · **204건/분**으로 5~10배 빨라졌다. 새 대량 수집을 붙일 때도 워커 증설이
  아니라 이 방식을 쓸 것. 주차만 필요할 땐 표제부를 `rows=1` 로(주차는 필지 단위 값이라
  동별 전체가 불필요 - 대단지 응답이 수십 배 작아진다).
