...
- ⚠ **공공데이터 API는 청약홈 웹 캘린더보다 며칠 늦다**(2026-07-23 확인): 웹
  캘린더에 뜬 공고가 API엔 아직 없을 수 있다(예: 7/27 해링턴플레이스 노원 센트럴
  임의공급, 7/28 아크로 리버스카이 무순위 — 서울 기준 API 최신 공고일이 각각
  임의공급 7/02·무순위 7/22 라 미수록). **누락처럼 보여도 대개 우리 수집 버그가
  아니라 원천 지연**이니, 의심되면 `ApplyhomeClient.fetch_notices` 로 직접
  조회해 API에 실제로 있는지부터 확인할 것. daily 수집이 올라오는 대로 채운다.
